...
{
    std::unique_lock lock{m_mutex};
    std::vector<Entry>::iterator anyIt = std::ranges::find_if(m_anyEntries, [h](const Entry& e) { return e.handle == h; });
    ...
    {
        std::vector<Entry>::iterator it = std::ranges::find_if(m_handlersById[i], [h](const Entry& e) { return e.handle == h; });
