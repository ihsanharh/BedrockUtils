...
public:
    ...
    union {
        ...
        int64_t uniqueEntityId;   // +0x60 Unique entity ID alias
    };
    ...
    // Helpers
    std::string_view getNametag() const { return entityData.getString(EntityDataIDs::NAME); }
    std::string getUuidString() const { return SDK::uuidToString(uuid); }

    virtual ~AddPlayerPacket() = default;
    PacketID getID() const override { return PacketID::ADD_PLAYER; }
};
